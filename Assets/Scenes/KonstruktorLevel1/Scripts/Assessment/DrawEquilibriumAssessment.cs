...
public class DrawEquilibriumAssessment : ResultAssessment
{
  ...
  public DrawEquilibriumAssessment(KonstruktorSceneData data, VariableInfoSO info) : base(data, info) {
  ...
  public override int CalculatePoints() {
    ...
    switch(isOnActionLine) {
      ...
      case true when isOppositeDirection == true:
        ...
        currentFeedback = FeedbackType.DrawEquilibriumOppositeOnActionLine;

        return 1;

      case true when isOppositeDirection == false:

        currentFeedback = FeedbackType.DrawEquilibriumNotOppositeOnActionLine;

        return 0;

      case false when isOppositeDirection == true:

        currentFeedback = FeedbackType.DrawEquilibriumOppositeNotOnActionLine;

        return 0;

      case false when isOppositeDirection == false:

        currentFeedback = FeedbackType.DrawEquilibriumNotOppositeNotOnActionLine;
